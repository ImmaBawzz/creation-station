...
Release deletion, release recreation, and `v1.7.0-alpha.2` release creation remain blocked until the owner gives explicit approval. README, CI, release notes, validation, release branch push, and PR creation are approved in the current release-prep cycle. The current release-prep PR passed CI after the npm 10 `package-lock.json` synchronization, route context type, and FFprobe test fixes; merge, tag, and release decisions still require separate approval.
