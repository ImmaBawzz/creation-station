...
- Release prep PR #1 is open and CI has passed; wait for review and explicit approval before any merge, tag, or release action.
...
- Earlier PR CI runs failed at `npm ci`, `npx tsc --noEmit`, and `npm test`; fixes were pushed and a later run passed.
